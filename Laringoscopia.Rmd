...
Una vez finalizado el estudio y recabada la información de los pacientes, estos fueron colocados en la base de datos "Laryngoscope" en la paquetería de medicaldata en el Software Estadístico R.
...
```{r}
list.of.packages <- c("medicaldata", "janitor","dplyr","ggplot2","cowplot","gmodels","rstatix","psych","RColorBrewer","ggpubr","car","stats", "corrplot", "tidyverse","MVN","DescTools","broom","survival","survminer","vembedr", "kableExtra", "gtsummary")
...
<div align="justify">
En el código debajo se crean dos variables auxiliares `Intentos_factor` y `Mallampati_factor`, basta con mencionar el nombre de la base de datos seguido de un **$** y el nombre de la nueva variable.
...
que nos servirán más delante en análisis estadístico, esto basta con 
</div>
...
La aleatorización se basó en datos generados por computadora y códigos de bloques aleatorios que fueron mantenidos dentro de sobres opacos enumerados. 
...
### Estadística descriptiva {.tabset .tabset-pills}

<div align="justify">
La función `summary()` calcula los parámetros de cada una de las variables introducidas siempre y cuando éstas sean tipo numéricas, por otro lado, cuando las variables son tipo factor, ésta función devuelve un conteo de las frecuencias por niveles. 
</div>
...
```{r}
datos %>%
  select("Grupo","Tiempo_total_intubacion","Nivel_dificultad","Exito_1_intento", "Exito_intubacion", "Num_intentos", "Vista_glotis", "Sangrado", "Dolor_garganta") %>%
  tbl_summary(by = Grupo,
              statistic = list(
                all_categorical() ~ "{n} ({p}%)")) %>% add_p() %>% add_overall() %>%
...
macin_estadi <- datos %>%
  ...
  select("Tiempo_1_intento","Tiempo_2_intento","Tiempo_3_intento","Tiempo_total_intubacion","Nivel_dificultad") %>%
  summary()
```

```{r echo=FALSE}
# Convertir en tabla y darle formato

macin_estadi %>%
  as.table() %>%
  kbl(caption = "Tabla de estadísticas descriptivas en grupo Macintosh") %>%
...
pentax_estadi %>%
  as.table() %>%
  kbl(caption = "Tabla de estadísticas descriptivas en grupo Pentax AWS") %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```

<div align="justify">
`r macin_estadi[3,5]` [`r macin_estadi[2,5]`, `r macin_estadi[5,5]`]
...
Una alternativa para obtener las estadísticas descriptivas de las variables numéricas, es la función `describeBy()`, dentro de la cual se puede indicar que se desea realizar la agrupación por tratamiento y ésta devuelve los cálculos de los siguientes parámetros:
...
```{r, class.output="scroll-100"}
# Estadística decriptiva utilizando función describeBy

descri <- describeBy(datos[,c(7, 9, 12, 17, 20)], group = datos$Grupo)
...
<div align="justify">
En el grupo Macintosh no hubo ningun tercer intento de intubación.
</div>
...
vector1 <- c("Edad","Genero","IMC","Mallampati","Grupo","Tiempo_1_intento", "Exito_1_intento", "Tiempo_2_intento", "Metodo_2_intento", "Exito_2_intento", "Tiempo_3_intento", "Metodo_3_intento", "Exito_3_intento", "Num_intentos", "Fracasos", "Tiempo_total_intubacion","Exito_intubacion", "Sangrado", "Nivel_dificultad", "Dolor_garganta", "Vista_glotis")
