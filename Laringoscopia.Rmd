...
tabla1 <- datos %>%
          ...
          select("Grupo","Tiempo_total_intubacion", "Nivel_dificultad","Exito_1_intento", 
                 "Exito_intubacion", "Num_intentos","Vista_glotis", "Sangrado", 
                 "Dolor_garganta") %>%
          ...
          # Cambio de nombre de la columna de las variables 
...
tabla2 <- datos %>%
          ...
          select("Grupo", "Edad", "Genero", "IMC", 
                 "Estado_fisico", "Mallampati_factor") %>%
  
          tbl_summary(by = Grupo,                                   
                      missing = "no",                               
                      statistic = list(c("Edad", "IMC", ) ~ "{mean} ({sd})",
                                       c("Estado_fisico", "Mallampati_factor", "Genero", "Grupo")~ "{n} ({p}%)")) %>%  
...
```
Los histogramas podemos crearlos usando la paquetería `ggplot()`, la cual cuenta con diferentes métodos 

```{r}
# Histogramas de las variables respuesta cuantitativas

h1 <- ggplot(datos, aes(x = Tiempo_total_intubacion)) +       #
      geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "white") +
      geom_density(col=2, lwd=1) +
      facet_wrap(~ Grupo)
      
h2 <- ggplot(datos, aes(x = Num_intentos)) + 
      geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "white") +
      geom_density(col=2, lwd=1) +
      facet_wrap(~ Grupo)
      
h3 <- ggplot(datos, aes(x = Nivel_dificultad)) + 
      geom_histogram(aes(y = ..density..),
                 colour = 1, fill = "white") +
      geom_density(col=2, lwd=1) +
      facet_wrap(~ Grupo)
...
```{r}
ggarrange(h1, h2, h3,  labels = c("A","B","C"),
          heights = c(10, 10, 10), widths = c(3, 3, 3),
